...
| COMBO-001 | COMBO | ComboBox ist ein editierbares ARIA-Combobox-Control mit nativer Input-Semantik und Popup-Optionen. | CONFIRMED | User decision, 2026-09-21 | Bodywork-Geometrie und Zustände bleiben DESIGN REVIEW. |
...
| COMBO-005 | COMBO | ComboBox unterstützt Single-Select und Multi-Select über `multiple?: boolean`; im Multi-Select liefern `value`/`defaultValue` und `onValueChange` die ausgewählten `ComboBox.Option`-Elemente. | CONFIRMED | User decision, 2026-09-21 | Bodywork-Geometrie und Zustände bleiben DESIGN REVIEW. |
| BADGE-001 | BADGE | Badge ist ein Status-Element mit dekorativer Standardsemantik; `aria-label` setzt `role="status"`. Die bestätigten Tones sind `neutral`, `accent`, `success`, `warning`, `danger`; Größen sind `sm` und `md`. | CONFIRMED | User decision, 2026-09-21 | Radius und exakte Geometrie bleiben DESIGN REVIEW und blockieren die Implementierung. |
| BADGE-002 | BADGE | Badge unterstützt optional `onRemove` mit verpflichtendem lokalisiertem `removeLabel`; die native Remove-Aktion wird vom Consumer kontrolliert und blendet die Badge nicht selbst aus. | CONFIRMED | User decision, 2026-09-21 | Bodywork-Geometrie und Close-Anatomy bleiben DESIGN REVIEW. |
