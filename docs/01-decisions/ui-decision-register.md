...
| SKEL-001 | SKELETON | Skeleton ist ein dekorativer Core-Platzhalter für Inhalte, die in ein bereits bekanntes Layout geladen werden. Unbekannte Ladezeiten verwenden Spinner; bekannter Fortschritt verwendet einen Balken mit sichtbarer Prozentangabe. | CONFIRMED | User decision, 2026-09-23; Bodywork `Fortschritt` | |
| SKEL-002 | SKELETON | Skeleton ist immer dekorativ, setzt `aria-hidden="true"` und kommuniziert keinen eigenen Lade-Status. Der umgebende Ladebereich verantwortet notwendige Statuskommunikation. | CONFIRMED | User decision, 2026-09-23 | |
| SKEL-003 | SKELETON | Skeleton bietet `square`, `rounded` und `circular` als Formen: 0, 6px beziehungsweise 50% Radius. `rounded` ist Standard. | CONFIRMED | User decision, 2026-09-23; Bodywork `Fortschritt` | |
| SKEL-004 | SKELETON | Skeleton definiert keine Größen-Props. Consumer legen Breite und Höhe mit nativen `style`- oder `className`-Props fest; `circular` verlangt gleiche Breite und Höhe. | CONFIRMED | User decision, 2026-09-23 | |
| SKEL-005 | SKELETON | Skeleton verwendet den Bodywork-Shimmer mit `--surface-alt`, `--border-soft`, 800px Background-Size, -400px bis 400px Background-Position sowie 1.4s `ease` und unendlicher Wiederholung. Diese bestätigte component-spezifische Background-Position-Animation ist eine Ausnahme von MOTION-002. | CONFIRMED | User decision, 2026-09-23; Bodywork `Fortschritt` | |
| SKEL-006 | SKELETON | Bei `prefers-reduced-motion: reduce` wird Skeleton statisch gerendert und der Shimmer deaktiviert. | CONFIRMED | User decision, 2026-09-23 | |
| SPIN-001 | SPINNER | Spinner ist ein dekorativer Core-Indikator für Ladezeiten mit unbekannter Dauer. Der umgebende Ladebereich verantwortet Statuskommunikation und Accessible Name. | CONFIRMED | User decision, 2026-09-24; Bodywork `Fortschritt` | |
| SPIN-002 | SPINNER | Spinner rendert einen nicht interaktiven `div` mit erzwungenem `aria-hidden="true"`, ohne Children, Rolle oder Fokusziel. Kompatible übrige native `div`-Props und der Ref werden weitergereicht. | CONFIRMED | User decision, 2026-09-24 | |
| SPIN-003 | SPINNER | Spinner hat keine Größen-Prop und immer `width` und `height` von `var(--k30)`. | CONFIRMED | User decision, 2026-09-24; Bodywork CSS | |
| SPIN-004 | SPINNER | Spinner verwendet einen 3px-Ring mit `--surface-alt` und einem oberen akzentfarbenen Viertel mit `--accent`. | CONFIRMED | User decision, 2026-09-24; Bodywork CSS | |
| SPIN-005 | SPINNER | Spinner verwendet die Bodywork-Animation `dsSpin 0.8s linear infinite` mit `transform: rotate(360deg)`. Bei `prefers-reduced-motion: reduce` bleibt er statisch sichtbar. | CONFIRMED | User decision, 2026-09-24; Bodywork CSS | |
| PROG-001 | PROGRESS | Progress ist ein Core-Indikator für bekannten Fortschritt. Er erhält einen ganzzahligen Prozentwert von 0 bis 100, zeigt diesen immer sichtbar an und ist kein indeterminierter Indikator. | CONFIRMED | User decision, 2026-09-24; Bodywork `Fortschritt` | |
| PROG-002 | PROGRESS | Progress erhält eine verpflichtende, bereits lokalisierte `label: string`-Prop und zeigt Label sowie Prozentwert oberhalb des Balkens. | CONFIRMED | User decision, 2026-09-24; Bodywork `Fortschritt` | |
| PROG-003 | PROGRESS | Progress verwendet ein eigenes `div` mit `role="progressbar"`. Die Komponente besitzt Accessible Name und ARIA-Werte vollständig: Das sichtbare Label wird per interner ID über `aria-labelledby` verknüpft und `aria-valuemin`, `aria-valuemax`, `aria-valuenow` sowie `aria-valuetext` werden selbst gesetzt. | CONFIRMED | User decision, 2026-09-24 | |
| PROG-004 | PROGRESS | Direkte kompatible native `div`-Props und Ref gehören zum Progressbar-Control. Ein optionales benanntes `rootProps` adressiert den umschließenden Layout-Root. | CONFIRMED | User decision, 2026-09-24 | |
| PROG-005 | PROGRESS | Progress rundet endliche Dezimalwerte auf die nächste ganze Zahl und begrenzt anschließend auf 0 bis 100. `NaN` wird als 0 behandelt; positive und negative Unendlichkeit werden auf 100 beziehungsweise 0 begrenzt. | CONFIRMED | User decision, 2026-09-24 | |
| PROG-006 | PROGRESS | Progress füllt die verfügbare Breite. Der Balken ist 8px hoch, vollständig gerundet, verwendet `--surface-alt` als Track und `--accent` als Füllung. Wertänderungen werden nicht animiert. Die Label-Zeile nutzt `display: flex`, `justify-content: space-between`, `font-size: var(--fs-meta)`, `color: var(--muted)` und `margin-bottom: 7px`. | CONFIRMED | User decision, 2026-09-24; Bodywork CSS | |

## Date and Time Picker (planned)

| ID | Category | Decision | Status | Source | Follow-up |
|---|---|---|---|---|---|
| PICKER-001 | PICKER | Der geplante Core Picker bietet genau die Modi `time` und `date`; `time` ist der Standard. Ein kombinierter DateTime-Modus ist nicht Teil dieses Vertrags. | CONFIRMED | User decision, 2026-09-22 | Öffentliche API in der Component Specification festlegen. |
| PICKER-002 | PICKER | `time` wählt Stunde und Minute über zwei Wheels. Minuten sind standardmäßig in Ein-Minuten-Schritten von 00 bis 59 auswählbar; `minuteStep` erlaubt zusätzlich 5-, 15- oder 30-Minuten-Schritte. Ein nicht passender Consumer-Wert wird beim Rendern ohne `onChange` auf den vorherigen gültigen Schritt normalisiert. `date` wählt Tag, Monat und Jahr über drei Wheels. | CONFIRMED | User decision, 2026-09-22/23 | Wheel-Anatomy, DOM und Token-Mapping spezifizieren. |
| PICKER-003 | PICKER | Die kontrollierte Value-API verwendet `Date | null` und `onChange(date)`. Bei `null` erhält der Trigger einen verpflichtenden lokalisierten `placeholder`. Ein unkontrollierter Modus ist nicht vorgesehen; bei der ersten Wheel-Auswahl dient die aktuelle lokale Zeit als Basis. | CONFIRMED | User decision, 2026-09-22/23 | Exakte Props und Event-Reihenfolge spezifizieren. |
| PICKER-004 | PICKER | Der Date-Modus verlangt eine BCP-47-`locale`-Prop. Datumsteile und Monatsnamen folgen dieser Locale; Werte werden in der lokalen Browser-Zeitzone interpretiert. `locale` ist auch im Zeitmodus verpflichtend; bei 12-Stunden-Locales ergänzt ein drittes, lokalisiertes Tagesperioden-Wheel die Stunden- und Minuten-Wheels. | CONFIRMED | User decision, 2026-09-22/23 | DST- und Fehlervertrag spezifizieren. |
| PICKER-005 | PICKER | Ohne Grenzen enthält das Jahr-Wheel das aktuelle Jahr plus/minus 100 Jahre. Optionale `minDate` und `maxDate` begrenzen ausschließlich den Date-Modus. Wird eine Datums-Kombination ungültig, wird der Tag auf den letzten gültigen Tag des gewählten Monats begrenzt. Ein Consumer-`value` außerhalb der Grenzen wird beim Rendern ohne `onChange` auf die nächstgelegene Grenze normalisiert; `minDate > maxDate` löst einen klaren Laufzeitfehler aus. | CONFIRMED | User decision, 2026-09-22/23 | Leap-Year-Fälle spezifizieren. |
| PICKER-006 | PICKER | Eine Auswahl löst `onChange` sofort aus und lässt das Dropdown geöffnet. Der Trigger, Escape oder ein Klick außerhalb schließen es; Fokus kehrt zum Trigger zurück. Wheels unterstützen Arrow Up/Down, Home/End und Tab-Navigation zwischen Wheels. | CONFIRMED | User decision, 2026-09-22 | Vollständigen Fokus-, Pointer-, Scroll-/Touch- und Screenreader-Vertrag spezifizieren. |
| PICKER-007 | PICKER | Die Wheels werden in einem Dropdown gerendert. Ein horizontaler Auswahlbalken liegt zentral über den Wheels; er markiert die ausgewählten Werte mit `--accent` bei 10% Opazität. | CONFIRMED | User decision, 2026-09-22 | Konkrete Geometrie und Token-Ausdruck spezifizieren. |
| PICKER-008 | PICKER | Der vorimplementierte Storybook-POC ist bis zu einer späteren Bodywork-Referenz die verbindliche visuelle Referenz für Trigger, Dropdown, Auswahlbalken und zyklische perspektivische Wheels. Die dokumentierte Ausnahme erlaubt die produktive Core-Implementierung ohne Bodywork-Eintrag. | CONFIRMED | User decision, 2026-09-23 | Bodywork-Übernahme später gegen diesen Vertrag prüfen. |
| PICKER-009 | PICKER | DateTimePicker erhält ein verpflichtendes sichtbares Floating-`label` und einen verpflichtenden lokalisierten `placeholder`. Das Label liefert den Accessible Name des nativen Trigger-Buttons. Die Komponente nimmt nicht an nativen Formular-Submits teil und bietet keinen `name`-Prop. | CONFIRMED | User decision, 2026-09-23 | Native-Prop- und DOM-Vertrag spezifizieren. |
| PICKER-010 | PICKER | Auf dem nativen Trigger wird der Consumer-`onClick` vor der internen Toggle-Logik aufgerufen; `event.preventDefault()` verhindert das Öffnen oder Schließen. | CONFIRMED | User decision, 2026-09-23 | Event-Komposition spezifizieren. |
