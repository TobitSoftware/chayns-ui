...
| COMBO-004 | COMBO | ComboBox verwendet string-basierte `value`-/`defaultValue`-Props und `onValueChange(value: string)`. | CONFIRMED | User decision, 2026-09-21 | |
| BADGE-001 | BADGE | Badge ist ein nicht-interaktives Status-Element mit dekorativer Standardsemantik; `aria-label` setzt `role="status"`. Die bestätigten Tones sind `neutral`, `accent`, `success`, `warning`, `danger`; Größen sind `sm` und `md`. | CONFIRMED | User decision, 2026-09-21 | Radius und exakte Geometrie bleiben DESIGN REVIEW und blockieren die Implementierung. |
