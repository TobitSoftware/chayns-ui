...
| BADGE-002 | BADGE | Badge unterstützt optional `onRemove` mit verpflichtendem lokalisiertem `removeLabel`; die native Remove-Aktion wird vom Consumer kontrolliert und blendet die Badge nicht selbst aus. | CONFIRMED | User decision, 2026-09-21 | Bodywork trennt Status-Badges von interaktiven Chips; die chayns-UI-Erweiterung ist als explizite API dokumentiert. |
