...
Arrow Left/Up selects and focuses the previous enabled segment; Arrow Right/Down the next; both wrap. Home selects/focuses the first, End the last. Space and Enter activate the focused native button. Disabled segments are skipped. Selection updates immediately and calls `onValueChange`; a controlled parent then supplies the value. Bodywork supplies `--toggle-bg`, `--k4` interior gap/padding, 12px group radius, 8px selected surface radius and `--shadow-card`. The group flex-wraps for localized text and narrow layouts. There is no local size prop or external margin.
