...
* Focus uses `--focus-ring-size` and the resolved accent at `--focus-ring-alpha-strong` opacity as a `box-shadow` `:focus-visible` ring, matching the DesignSystem reference (`rgba(var(--focus-ring-rgb), var(--focus-ring-alpha-strong))`). The softened accent lets the ring stand off the button surface rather than reading as a thicker border. Forced-colors mode falls back to a solid `ButtonText` outline.
